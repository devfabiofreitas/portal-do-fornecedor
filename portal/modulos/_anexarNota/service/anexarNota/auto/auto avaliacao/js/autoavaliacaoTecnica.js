function AutoAvaliacaoTecnica() {
    var jsonTagsBanco = [];
    var atualizacao_avaliacao = false;
    var lider = "";
    var colaborador = '';
    var email = "";
    this.init = function() {
    ...
    this.verificarAcesso = function () {       
        ...
        main.executarAPI('', 'SELECT', '', JSON.stringify(params), function (obj) {
            if(obj.success) {
                ...
                var $table = $('#mainTable');
                colaborador = obj.elements[0].nome;
                $('#nomeConsultor').html(' - ' + obj.elements[0].nome);
                ...
                // autoAvaliacao.listCompetencias(obj.elements);                
            }else{
    ...
    this.listarResumoPerfil = function (dados) {       
        var dadosResumo = main.convertToJson(dados);
        
        lider = dadosResumo[0].lider;
        email = dadosResumo[0].email_Lider;

        // $('#nomeConsultor').html('/ <span class="font-cairo-bold">' + dadosResumo[0].nome + '</span> <a href="' + link + '" target="_blank">' + endereco + '</a>');
        ...
        if(dadosResumo[0].seguimento == '') {
            $('.seguimento').html('Nenhum seguimento foi encontrado.');
        }else{
            $('.seguimento').html(dadosResumo[0].seguimento);
        }
        ...
        for (var i = 0; i < tagsForm.tags.length; i++) {
            for (var j = 0; j < TagsBd.length; j++){
                if(tagsForm.tags[i].Tags_codigo == TagsBd[j].Tags_codigo){
                    if(TagsBd[j].existe != null){
                        // console.log(TagsBd[j],TagsBd[j].autoAvaliacao)
                        if(TagsBd[j].autoAvaliacao != null && TagsBd[j].autoAvaliacao.length > 0){
                            var idx = TagsBd[j].autoAvaliacao.length;
                            if(TagsBd[j].autoAvaliacao[idx -1] != null){
                                var alteracao = false;
                                if(tagsForm.tags[i].autoAvaliacao['estudo'] != TagsBd[j].autoAvaliacao[idx -1]['estudo']){
                                    alteracao = true;
                                }else if(tagsForm.tags[i].autoAvaliacao['projeto'] != TagsBd[j].autoAvaliacao[idx -1]['projeto']){
                                    alteracao = true;
                                }else if(tagsForm.tags[i].autoAvaliacao['senioridade'] != TagsBd[j].autoAvaliacao[idx -1]['senioridade']){
                                    alteracao = true;
                                }

                                if(alteracao){
                                    TagsBd[j].autoAvaliacao.push(tagsForm.tags[i].autoAvaliacao);
                                    tagsForm.tags[i].autoAvaliacao = TagsBd[j].autoAvaliacao;
                                    tagsForm.tags[i].autoAvaliacao[idx]['data_alteracao'] = dateNow;
                                    jsonAlteracao.push(tagsForm.tags[i])
                                }

                            }else{
                                var alteracao = false;
                                if(tagsForm.tags[i].autoAvaliacao['estudo'] != TagsBd[j].autoAvaliacao['estudo']){
                                    alteracao = true;
                                }else if(tagsForm.tags[i].autoAvaliacao['projeto'] != TagsBd[j].autoAvaliacao['projeto']){
                                    alteracao = true;
                                }else if(tagsForm.tags[i].autoAvaliacao['senioridade'] != TagsBd[j].autoAvaliacao['senioridade']){
                                    alteracao = true;
                                }

                                if(alteracao){
                                    TagsBd[j].autoAvaliacao = [TagsBd[j].autoAvaliacao];
                                    TagsBd[j].autoAvaliacao.push(tagsForm.tags[i].autoAvaliacao);
                                    tagsForm.tags[i].autoAvaliacao = TagsBd[j].autoAvaliacao;
                                    tagsForm.tags[i].autoAvaliacao[1]['data_alteracao'] = dateNow;
                                    jsonAlteracao.push(tagsForm.tags[i])
                                }
                            }
                        }else{
                            if(tagsForm.tags[i].autoAvaliacao['senioridade'] > 0){
                                ...
                                tagsForm.tags[i].autoAvaliacao[0]['data_alteracao'] = dateNow;
                                jsonAlteracao.push(tagsForm.tags[i])
                            }
                        }
                    }else{
    ...
    this.cadastrar = function () {
        ...
        var params = [];
        params = {tags: paramsTags, competencias: paramsCompetencia, codigo}     
    ...
    this.getDadosSenioridade = function(val){
        ...
        switch (val) {
            ...
            case 3:
                titulo = 'Nível 3';
                texto = 'Já atuou em preojetos. Requer acompanhamento de profissional nos produtos entregues.';
                cor = '#f3701f';
            break;
            case 4:
                titulo = 'Nível 4';
                texto = 'Já atuou em projetos. Capaz de realizar atividades de complexidade baixa de forma idependente.';
                cor = '#f68f1c';
            break;
            case 5:
                titulo = 'Nível 5';
                texto = 'Consegue executar atividades de nível moderado de forma idependente.';
                cor = '#ffc10f';
            break;
            case 6:
                titulo = 'Nível 6';
                texto = 'Pleno conhecimento das aplicações da Ferramenta/Liguagem. Encabeça atividades junto ao cliente.';
                cor = '#fef100';
            break;
            case 7:
                titulo = 'Nível 7';
                texto = 'Interpreta requisitos de forma idependente. Experiência adquirida em vários projotes; Provê treinamentos internos. Gera conteúdo para capacitação de outros colaboradores.';
                cor = '#8ffd09';
            break;
            case 8:
                titulo = 'Nível 8';
                texto = 'Atua de forma idependente. Participa de reuniões de forma ativa, capaz de sugerir soluções.';
                cor = '#00a990';
            break;
            case 9:
                titulo = 'Nível 9';
                texto = 'Certificado oficial de mercado. Representa a empresa em ações comerciais, através de Palestras Worshops e Treinamentos. Atua em questões de arquiteturas e soluções em suas especialidades.';
                cor = '#0093ce';
            break;        
            default:
                texto = 'Nenhum conhecimento';
