function LogicaReport(){
    ...
    this.parseHoras = function(){};

    this.getProjetos = function(data){
      let projetos = data.reduce((acumulador, elemento) => {
        acumulador[elemento.projeto] = ( acumulador[elemento.projeto] || 0) + 1;
        return acumulador} , [])
    ...
    this.maiorFrequenciaCliente = function(data){
      let clientes = data.reduce((acumulador, elemento) => {
        acumulador[elemento.cliente] = ( acumulador[elemento.cliente] || 0) + 1;
        return acumulador} , [])
        ...
        for(let i = 0; i < data.length; i++){
          ...
          countIf = 0;
           if(data[i] instanceof Array ){
            for(let z in data[i]){
              if(data[i][z].projeto === projetos[x]){
                if(countElse){
                  datas[x].pop();
                  countElse = 0;
                }
