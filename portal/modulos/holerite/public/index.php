...
$usuario_nome = $usuario_sessao['nome'];
...
if (!empty($_POST['mesAno']) && $usuario_sessao) {
    $info_holerite = listar_holerite_filtro($_POST['mesAno'], $usuario_codigo, $do);
} else {
    $info_holerite = listar_holerite($usuario_codigo, $do);
}


$template_tabela = gerar_tabela($info_holerite, $usuario_nome);


$alerta_swal = (isset($_REQUEST['status'])) ? exibir_alerta_javascript($_REQUEST['status'], $_REQUEST['message']) : '';

require_once('../templates/holerite.template.php');
