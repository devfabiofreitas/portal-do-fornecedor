function Timesheet(){
  ...
  $('#formDados').submit(async function(event) {
    ...
    if (main.validarCampos('#formDados')) {
      ...
      var botaoClicado = $(this).find('button[type="submit"]:focus');

    // Obter o valor e o nome do botão de envio
      var valorBotao = botaoClicado.val();
      var nomeBotao = botaoClicado.attr('name');
    $(this).append('<input type="hidden" name="' + nomeBotao + '" value="' + valorBotao + '">');

      $("#Projeto_codigo").val($("#Project").find(':selected').attr("data-Projeto"));
      $("#Cliente_codigo").val($("#Project").find(':selected').attr("data-Client_cod"));
      $("#Faturavel").val($("#Project").find(':selected').attr("data-Billable"));
      $("#Tags_join").val($("#Tags").val().join(', '));
        if(!await timesheet.verificaCadastroFuturo()){ //ok
          loading.close();
          return;
        }

        if(!await timesheet.verificaCadastroPassado()){ //ok
          loading.close();

          return;
        }

        if(await timesheet.verificarRecessoRemunerado()){ //ok
          loading.close();
  ...
  $("#reportTimesheet").click(function () {
    window.location.href = "../../timesheetReport/fragment/reportTimesheet.frag.php";
  });
  ...
  */
  ...
  this.controleTimesheetDataSas = function () {
    ...
    if(dataHojeDia > 20){
      ...
      fechamentoSas = moment(dataHojeAno + "-" + (dataHojeMes) + "-" + diaFechamentoSas).add(1, "M").format("YYYY-MM-DD"); //20/09/2022
    } else {
      aberturaSas = moment(dataHojeAno + "-" + (dataHojeMes) + "-" + diaAberturaSas).subtract(1,'months').format("YYYY-MM-DD") ; // 21/07/2022
