...
// echo $http_headers['senha'] . ' ---- ';
// $http_headers['senha'] = utf8_encode($http_headers['senha']);
// echo $http_headers['senha'];
if (!isset($http_headers['usuario']) || $http_headers['usuario'] == '' || !isset($http_headers['senha']) || $http_headers['senha'] == '') {
	echo '{"success": false, "mensagem": "Dados de login não enviados."}';
	exit;
}
// echo $http_headers['senha'];
$usuario = $http_headers["usuario"];
$senha = hash('sha512', $http_headers["senha"]);
// echo $http_headers['senha'];
